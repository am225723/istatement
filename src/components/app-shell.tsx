...
const tabs: { id: Tab; label: string; icon: any }[] = [
  ...
  { id: 'seen', label: 'SEEN Method', icon: Eye },
...
export function AppShell() {
  ...
  async function sendFollowUp() {
    ...
    try {
      const res = await fetch('/api/ai', {
        ...
        body: JSON.stringify({
          message: `The user is refining an I-statement. Previous AI output:\n${reply}\n\nUser clarification:\n${userMessage}\n\nRevise the I-statement based on the clarification. End by asking if the revision is closer to what they meant.`
        })
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'AI request failed');
  ...
  async function generateSeen() {
    ...
    try {
      const res = await fetch('/api/ai', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ mode: 'seen', situation: seenSituation, reaction: seenReaction, desiredOutcome: seenOutcome })
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'AI request failed');
  ...
  async function saveJournal() {
    ...
    setJournalBody(''); setJournalTitle(''); await loadJournal();
  }
  ...
  return (
    <div className="mx-auto grid max-w-7xl gap-6 md:grid-cols-[260px_1fr]">
      <aside className="rounded-3xl bg-white/80 p-3 shadow-soft backdrop-blur">
        ...
        {tabs.map(({ id, label, icon: Icon }) => <button key={id} onClick={() => setTab(id)} className={`mb-2 flex w-full items-center gap-3 rounded-2xl px-4 py-3 text-left font-bold transition ${tab === id ? 'bg-ink text-white' : 'hover:bg-lavender'}`}><Icon className="h-5 w-5" /> {label}</button>)}
      </aside>

      <section className="rounded-3xl bg-white/85 p-5 shadow-soft backdrop-blur md:p-8">
        {tab === 'builder' && <div className="grid gap-6 lg:grid-cols-2">
          <div className="space-y-5">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <ModeCard active={builderMode === 'structured'} title="Structured I-Statement" description="Use guided fields: feeling, situation, impact, and request." onClick={() => setBuilderMode('structured')} />
              <ModeCard active={builderMode === 'raw'} title="Raw Message" description="Type what you really want to say and let AI reframe it without requiring extra fields." onClick={() => setBuilderMode('raw')} />
            </div>

            {builderMode === 'raw' ? <Field label="What do you really want to say?"><textarea value={raw} onChange={e=>setRaw(e.target.value)} placeholder="Type the unfiltered message here." className="h-48 w-full rounded-2xl border p-4 text-base" /></Field> : <>
...
function ChatBox({ chat, followUp, setFollowUp, sendFollowUp, loading }: any) { return <div className="rounded-3xl bg-white p-5 shadow-sm"><h3 className="font-black text-plum">Refine this message</h3><div className="mt-3 space-y-2">{chat.map((m: any, i: number)=><div key={i} className={`rounded-2xl p-3 text-sm ${m.role === 'user' ? 'bg-ink text-white' : 'bg-lavender text-slate-700'}`}>{m.text}</div>)}</div><textarea value={followUp} onChange={(e)=>setFollowUp(e.target.value)} placeholder="Tell the AI what it missed, what feels off, or what you want it to sound like..." className="mt-4 h-24 w-full rounded-2xl border p-3" /><button onClick={sendFollowUp} disabled={loading || !followUp.trim()} className="mt-3 rounded-2xl bg-rose px-5 py-3 font-black text-white disabled:opacity-60">Send clarification</button></div>; }
function InfoCard() { return <div className="rounded-3xl bg-lavender p-5 text-sm leading-6 text-slate-700"><b>S - Scared:</b> What threat am I perceiving?<br/><b>E - Embarrassed:</b> Am I feeling exposed, judged, or ashamed?<br/><b>E - Expectations:</b> What did I expect, and how is reality different?<br/><b>N - Need:</b> What do I need right now to move forward?</div>; }
function ModeCard({ active, title, description, onClick }: { active: boolean; title: string; description: string; onClick: () => void }) { return <button onClick={onClick} className={`rounded-3xl border p-5 text-left transition ${active ? 'border-rose bg-blush shadow-soft' : 'border-slate-200 bg-white hover:border-rose/40'}`}><span className="text-lg font-black text-ink">{title}</span><span className="mt-2 block text-sm leading-6 text-slate-600">{description}</span></button>; }
function PanelTitle({ title, subtitle }: { title: string; subtitle: string }) { return <div><h2 className="text-3xl font-black text-ink">{title}</h2><p className="mt-2 text-slate-600">{subtitle}</p></div>; }
...
function Output({ title, text }: { title: string; text: string }) { return <div className="rounded-3xl border border-plum/10 bg-gradient-to-br from-lavender to-blush p-6"><h3 className="text-xl font-black">{title}</h3><pre className="mt-4 whitespace-pre-wrap font-sans leading-7 text-slate-700">{text}</pre></div>; }
